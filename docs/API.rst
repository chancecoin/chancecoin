...
The Chancecoin API aims to be as simple and flexible as possible. To this end, it includes a straightforward
...
.. py:function:: get_messages(block_index)
   ...
   Return message feed activity for the specified block index. The message feed essentially tracks all chancecoind
...
.. py:function:: get_running_info()
   ...
   Gets some operational parameters for chancecoind.
   
   :return: An object with the following parameters:
     - **db_caught_up** (*boolean*): ``true`` if chancecoind block processing is caught up with the Bitcoin blockchain, ``false`` otherwise.  
     - **last_block** (*integer*): The index (height) of the last block processed by chancecoind
     - **chancecoind_version** (*float*): The chancecoind program version, expressed as a float, such as 0.5
     - **db_version_major** (*integer*): The major version of the current chancecoind database
     - **db_version_minor** (*integer*): The minor version of the current chancecoind database
...
An object that describes a specific event in the chancecoind message feed (which can be used by 3rd party applications
to track state changes to the chancecoind database on a block-by-block basis).
...
* **category** (*string*): A string denoting the entity that the message relates to, e.g. "credits", "burns", "debits".
  The category matches the relevant table name in chancecoind (see blocks.py for more info).
