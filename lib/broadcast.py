...
Broadcast a message, with or without a price.
 messages per block are allowed. Bets are be made on the 'timestamp'
...
def parse (db, tx, message):
    ...
    for bet_match in broadcast_parse_cursor.fetchall():
        ...
        call_type_id = util.BET_TYPE_ID['LongCall'] + util.BET_TYPE_ID['ShortCall']
        put_type_id = util.BET_TYPE_ID['LongPut'] + util.BET_TYPE_ID['ShortPut']
        ...
        # Call[/Put] bet.
        if validity == 'valid' and (bet_match_type_id == call_type_id or bet_match_type_id == put_type_id) and timestamp >= bet_match['deadline']:
            # Recognise tx0, tx1 as the bull, bear (in the right direction).
            if bet_match['tx0_bet_type'] < bet_match['tx1_bet_type']:
                bull_address = bet_match['tx0_address']
                bear_address = bet_match['tx1_address']
            ...
            target_value = bet_match['target_value']

            if bet_match_type_id == call_type_id:
                bear_credit = D(bear_escrow) - (max(0,D(value) - D(target_value))) * D(config.UNIT)
                bull_credit = D(total_escrow) - bear_credit
                bear_credit = round(bear_credit)
                bull_credit = round(bull_credit)
            elif bet_match_type_id == put_type_id:
                bear_credit = D(bear_escrow) - (max(0,D(target_value) - D(value))) * D(config.UNIT)
                bull_credit = D(total_escrow) - bear_credit
                bear_credit = round(bear_credit)
                bull_credit = round(bull_credit)
